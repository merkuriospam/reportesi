...
const ensureStructure = async () => {
  ...
  try {
    await sequelize.query(`ALTER TABLE \`Reports\` MODIFY COLUMN \`status\` ENUM('Pendiente', 'Atendido', 'Derivado', 'Resuelto') NULL DEFAULT 'Pendiente'`);
    console.log('Reports status ENUM updated');
  } catch (_) {}
